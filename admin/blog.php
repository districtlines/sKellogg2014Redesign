...
*/
		...
		if($_SERVER['REMOTE_ADDR'] == '97.103.133.213') {
			
/* 			echo $_SESSION['form_data']['show_date']; */
		...
		if($_POST['author']) {
			...
			$_SESSION['form_data']['author'] = $_POST['author'];
		
		} else {
			
			$_SESSION['form_error']['author'] = "Please supply an author for this post.";
		...
		if (!count($_SESSION['form_error'])) {
			...
			if ($id) {
				...
				$sql = "UPDATE blog SET show_date = '{$_SESSION['form_data']['show_date']}' WHERE id = $id";
						if($_SERVER['REMOTE_ADDR'] == '97.103.133.213') {
							//echo $sql;
						}
				mysql_query($sql);
				//ADD INTO RECENT ACTIVITIES	
				$typeSql = 'SELECT id FROM blog WHERE title = \'' . $_SESSION['form_data']['title'] . '\'';
				...
				{
					$sql = "SELECT sticky FROM blog WHERE id='$typeID'";
					...
					if($check)
					{
						while($row = mysql_fetch_assoc($query))
						{
							$sticky = $row['sticky'];
								...
								$sqlSticky = "INSERT INTO stickies (title,content,created,modified,type,type_id) values ('" . $_SESSION['form_data']['title'] . "','" . $_SESSION['form_data']['body'] . "', NOW(), NOW(), 'blog','" . $typeID . "')";
						}
					}
				}
				...
				if($_SESSION['form_data']['sticky'] != "1" && !$newEntry)
				{
						...
						$sqlSticky = "DELETE FROM stickies WHERE type = 'blog' AND type_id = '" . $typeID . "'";
						$querySticky = mysql_query($sqlSticky) or die(mysql_error());
				...
				if($_SESSION['form_data']['sticky'] == "1" && !$newEntry)
				{
					$sql = "SELECT * FROM stickies WHERE type_id='$typeID' and type='blog'";
					$query = mysql_query($sql);
					$check = mysql_num_rows($query);
					if($check)
					{
						$sqlSticky = "UPDATE stickies SET title = '" . $_SESSION['form_data']['title'] . "',content = '" . $_SESSION['body'] . "', created = NOW(), modified = NOW() WHERE type = 'blog' AND type_id = '$typeID'";
						$querySticky = mysql_query($sqlSticky) or die(mysql_error());
					}else{
						$sqlSticky = "INSERT INTO stickies (title,content,created,modified,type,type_id) values ('" . $_SESSION['form_data']['title'] . "','" . $_SESSION['form_data']['body'] . "', NOW(), NOW(), 'blog','" . $typeID . "')";
					$querySticky = mysql_query($sqlSticky) or die(mysql_error());
				...
				{
					$recentSql = "INSERT INTO recent_activity (title,content,created,modified,type,type_id,sticky) VALUES ('" . $_SESSION['form_data']['title'] . "','" . $_SESSION['form_data']['body'] . "', NOW(), NOW(),'blog','" . $typeID . "','" . $_SESSION['form_data']['sticky'] . "')";
		...
		if (is_array($_REQUEST['delete'])) {
			foreach ($_REQUEST['delete'] as $delete_id => $delete) {
				if ($delete) {
					...
					mysql_query("DELETE FROM recent_activity WHERE title='$name' AND type='blog' LIMIT 1") or die(mysql_error());
					mysql_query("DELETE FROM stickies WHERE title='$name' AND type='blog' LIMIT 1") or die(mysql_error());
				}
			}
...
<!-- 	<h3 style="color:#f00;">This Page is being worked on</h3> -->
...
/* 			$body = get_one('SELECT `body` FROM `content_pages` WHERE `id` = \'' . $_REQUEST['edit'] . '\''); */
		...
		<tr>
			<th>Author</th>
			<td class="<?= isset($_SESSION['form_error']['author']) ? ' error' : '' ?>">
...
?>
								...
								<option value="<?=$row['id']?>"<? echo $_SESSION['form_data']['author'] == $row['id'] ? ' selected="selected"' : '' ; ?>><?=ucwords($row['name'])?></option>
...
<?
    ...
    <tr>
		...
		<tr>
			<th>Body</th>
			<td class="<?= isset($_SESSION['form_error']['body']) ? ' error' : '' ?>"><textarea name="body" id="ignore_tf2"><?=stripslashes($_SESSION['form_data']['body']);?></textarea></td>
		</tr>
...
<tr>
			...
			<td class="<?= isset($_SESSION['form_error']['archived']) ? ' error' : '' ?> checkbox"><? input_checkbox('archived', '1') ?></td>
		</tr>
-->
		...
		<tr>
			...
			<td class="<?= isset($_SESSION['form_error']['showing']) ? ' error' : '' ?> checkbox"> <input type="checkbox"<? echo $_SESSION['form_data']['showing'] ? ' checked="yes"' : ''?> name="showing" value="1" />
</td>
		</tr>

<?
	...
	if ($_REQUEST['action'] == 'submit') {
		...
		if (!count($_SESSION['form_error'])) {
			...
			if ($id) {
				...
				if($check)
				{
					while($row = mysql_fetch_assoc($query))
					{
						$sticky = $row['sticky'];
							...
							$sqlSticky = "INSERT INTO stickies (title,created,modified,type,type_id) values ('" . $_SESSION['form_data']['name'] . "', NOW(), NOW(), 'photo','" . $typeID . "')";
					}
				}
				}
				...
				if($_SESSION['form_data']['sticky'] != "1" && !$newEntry)
				{
						...
						$sqlSticky = "DELETE FROM stickies WHERE type = 'photo' AND type_id = '" . $typeID . "'";
						$querySticky = mysql_query($sqlSticky) or die(mysql_error());
				...
				if($_SESSION['form_data']['sticky'] == "1" && !$newEntry)
				{
					$sql = "SELECT * FROM stickies WHERE type_id='$typeID' and type='photo'";
					$query = mysql_query($sql);
					$check = mysql_num_rows($query);
					if($check)
					{
						$sqlSticky = "UPDATE stickies SET title = '" . $_SESSION['form_data']['name'] . "', created = NOW(), modified = NOW() WHERE type = 'photo' AND type_id = '$typeID'";
						$querySticky = mysql_query($sqlSticky) or die(mysql_error());
					}else{
						$sqlSticky = "INSERT INTO stickies (title,created,modified,type,type_id) values ('" . $_SESSION['form_data']['name'] . "', NOW(), NOW(), 'photo','" . $typeID . "')";
					$querySticky = mysql_query($sqlSticky) or die(mysql_error());
				...
				{
					$recentSql = "INSERT INTO recent_activity (title,created,modified,type,type_id,sticky) VALUES ('" . $_SESSION['form_data']['name'] . "'," . "NOW(), NOW(),'photo','" . $typeID . "','" . $_SESSION['form_data']['sticky'] . "')";
		...
		redirect('./' . section . '.php?gallery_id=' . $album_id);
	}
	//delete
		...
		if (is_array($_REQUEST['delete'])) {
			foreach ($_REQUEST['delete'] as $delete_id => $delete) {
				if ($delete) {
					...
					mysql_query("DELETE FROM recent_activity WHERE title='$name' AND type='photo' LIMIT 1") or die(mysql_error());
					mysql_query("DELETE FROM stickies WHERE title='$name' AND type='photo' LIMIT 1") or die(mysql_error());
				}
			}
...
?>
	...
	<form action="./<?= section ?>.php?gallery_id=<?=$album_id?>" method="post" id="add_form" enctype="multipart/form-data">
	...
	<table id="form_table" cellspacing="0">
		...
		<tr>
			...
			<?php
			$sql = "SELECT name FROM gallery WHERE id = '". $album_id ."'";
			$query = mysql_query($sql) or die(mysql_error());
			$check = mysql_num_rows($query);
			if ($check) {
				$gallery_name = mysql_result($query, 0);	
			}
			echo "Add a photo to " . stripslashes($gallery_name);
			?>
			<input type="hidden" name="gallery_id" value="<?= $album_id ?>" />
			</td>
		</tr>
 ...
 name="showing" value="1" checked="yes" />
	...
	<form action="./<?= section ?>.php?gallery_id=<?=$album_id?>" method="post">
		...
		input_hidden('gallery_id', $album_id);
		...
		<table id="listing_table" cellspacing="0">
			...
			<?
				if (get_rows("SELECT * FROM $_table WHERE gallery_id='$album_id'  ORDER BY `sort` ASC"))  while ($row = get_rows()) {
			...
			<tr class="<?= $row_class ?>" id="listing_table_row_<?= $row['id'] ?>">
				...
				?>
				<td width="<?= floor(90 / count($_listing_fields)) ?>%"><a href="./<?= section ?>.php?edit=<?= $row['id'] ?>&gallery_id=<?= $album_id ?>"><?= shorten(strip_tags($row[$field]), floor(60 / count($_listing_fields))) ?></a></td>
				<? } ?>

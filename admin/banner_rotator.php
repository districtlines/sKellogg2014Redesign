		...
		if (is_numeric($_REQUEST['id'])) {
			...
			$image_sizes = getimagesize($_FILES['image']['tmp_name']);
				...
				if ($width < 280 || $height < 280) {
					$_SESSION['form_error']['image'] = "Your photo must be at least 280px by 280px.";
				}
		}
	...
	if ($_REQUEST['action'] == 'delete') {
		if (is_array($_REQUEST['delete'])) {
			foreach ($_REQUEST['delete'] as $delete_id => $delete) {
				if ($delete) {
					$sql = "SELECT image FROM ". $_table ." WHERE id = '$delete_id'";
...
?>
	...
	<table id="form_table" cellspacing="0">
        ...
        <tr>
        	<th>Photo<br /><small>280px x 280px</small></th>
	...
	<form action="./<?= section ?>.php" method="post">
		...
		input_hidden('album_id', $_REQUEST['album_id']);
		
		$sql = "SELECT id, name FROM banner_rotator";
		$query = mysql_query($sql) or die(mysql_error());
		...
		<table id="listing_table" cellspacing="0">
			...
			<tr class="<?= $row_class ?>" id="listing_table_row_<?= $row['id'] ?>">
				...
				<? if (is_array($_listing_fields)) foreach ($_listing_fields as $field) {
					...
					if ($field == "album_id") {
						$row[$field] = $photo_albums[$row[$field]];
					}
				?>
				<td width="<?= floor(90 / count($_listing_fields)) ?>%"><a href="./<?= section ?>.php?edit=<?= $row['id'] ?>&album_id=<?= $_REQUEST['album_id'] ?>"><?= shorten(strip_tags($row[$field]), floor(60 / count($_listing_fields))) ?></a></td>
				<? } ?>
			</tr>
